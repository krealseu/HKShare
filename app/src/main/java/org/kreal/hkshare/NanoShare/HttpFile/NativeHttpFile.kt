...
class NativeHttpFile(private val file: File, httppath: String) : HttpFile {
    ...
    constructor(nativeHttpFile: NativeHttpFile, filename: String)
            : this(File(nativeHttpFile.file, filename)
            , nativeHttpFile.uri + '/' + filename)
    ...
    override fun listfile(): Array<HttpFile> {
        ...
        return Array<HttpFile>(files.size) { i ->
